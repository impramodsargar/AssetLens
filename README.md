...
| **P8** live *(`-Probe`)* | **ACTIVE** - `httpx` liveness-probes in-scope URLs -> `08_live\live_urls.txt`/`live_uris.txt`; **fetches live JS** and re-extracts endpoints (+ `jsluice` **method/param API map** + the **deep pass**: DOM sinks / postMessage / GraphQL) + `trufflehog`/`gitleaks`/`retire.js` on the current code -> `live_js_*`. Opt-in; authorization required | active |
