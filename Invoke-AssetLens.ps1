...
function Invoke-JsluiceDeep {
    # jsluice AST "deep pass" via `query -f` (tree-sitter, so no string/comment false positives regex gives):
    #   DOM XSS sinks (+ tainted source), postMessage handlers (origin-check flagged), GraphQL operations.
    # Each finding is tagged with its SOURCE (the JS URL via $UrlMap[localpath], else the body filename).
    # Writes <OutDir>\<Prefix>{dom_sinks,postmessage,graphql_ops}.txt; returns a count summary. Batches files (argv cap).
    param([string]$JsluicePath, [string[]]$Files, [string]$OutDir, [string]$Prefix = '', [hashtable]$UrlMap = @{})
    $res = @{ sinks = 0; sinksHigh = 0; pm = 0; pmOpen = 0; gql = 0 }
    if (-not $Files -or @($Files).Count -eq 0) { return $res }
    function _Src { param($f) if ($UrlMap.ContainsKey("$f")) { $UrlMap["$f"] } else { [IO.Path]::GetFileName("$f") } }
    function _T { param($s) if ($s.Length -gt 160) { $s.Substring(0, 160) + ' ...' } else { $s } }
    function _JQ { param($q, $cap)
        # -f makes jsluice emit {"<cap>":"<node source>","filename":"<local body path>"} per match
        ...
        for ($i = 0; $i -lt $Files.Count; $i += 80) {
            ...
            $r = Invoke-Tool $JsluicePath (@('query', '-f', '-q', $q) + $chunk) -TimeoutSec 600
            if (-not $r) { continue }
            foreach ($ln in $r) {
                $obj = $null; try { $obj = "$ln" | ConvertFrom-Json } catch {}; if (-not $obj) { continue }
                $code = "$($obj.$cap)"; if (-not $code) { continue }
                [void]$o.Add([pscustomobject]@{ code = ($code -replace '\s+', ' ').Trim(); file = "$($obj.filename)" })
            }
        }
        return $o
    }
    # taint sources that raise a sink to [HIGH]. Matching is CASE-SENSITIVE (-cmatch) so 'Function' the constructor
    # is not confused with every 'function', and sinks are matched on the CALLEE (before the first '(') or the
    # assignment TARGET (before '='), so a giant enclosing IIFE that merely contains a sink deep inside is not flagged.
    ...
    $asns  = _JQ '(assignment_expression) @a' 'a'
    $calls = _JQ '(call_expression) @c' 'c'
    $sinks = New-Object System.Collections.Generic.List[string]
    foreach ($a in $asns) {
        $code = $a.code; $left = ($code -split '=', 2)[0]
        ...
        if ($cat) { $hi = [bool]((($code -split '=', 2)[-1]) -cmatch $taint); if ($hi) { $res.sinksHigh++ }; $res.sinks++; [void]$sinks.Add(('{0} {1,-13} {2}   <=  {3}' -f $(if ($hi) { '[HIGH]' } else { '[sink]' }), $cat, (_T $code), (_Src $a.file))) }
    }
    foreach ($c in $calls) {
        $code = $c.code; $callee = ($code -split '\(', 2)[0]; $rest = ($code -split '\(', 2)[-1]
        ...
        if ($cat) { $hi = [bool]($rest -cmatch $taint); if ($hi) { $res.sinksHigh++ }; $res.sinks++; [void]$sinks.Add(('{0} {1,-13} {2}   <=  {3}' -f $(if ($hi) { '[HIGH]' } else { '[sink]' }), $cat, (_T $code), (_Src $c.file))) }
    }
    ...
    foreach ($c in $calls) {
        $code = $c.code; $rest = ($code -split '\(', 2)[-1]
        if (($code -split '\(', 2)[0] -cmatch '\.addEventListener$' -and $rest -cmatch '^\s*["'']message["'']') {
            $open = -not ($code -cmatch '\.origin\b'); if ($open) { $res.pmOpen++ }; $res.pm++
            [void]$pm.Add(('{0} {1}   <=  {2}' -f $(if ($open) { '[NO-ORIGIN-CHECK]' } else { '[checks-origin] ' }), (_T $code), (_Src $c.file)))
        }
    }
    foreach ($a in $asns) {
        $code = $a.code
        if (($code -split '=', 2)[0] -cmatch '\.onmessage\s*$') {
            $open = -not ($code -cmatch '\.origin\b'); if ($open) { $res.pmOpen++ }; $res.pm++
            [void]$pm.Add(('{0} {1}   <=  {2}' -f $(if ($open) { '[NO-ORIGIN-CHECK]' } else { '[checks-origin] ' }), (_T $code), (_Src $a.file)))
        }
    }
    # GraphQL operations from template literals (lowercase keyword + a brace)
    $gql = New-Object System.Collections.Generic.List[string]
    foreach ($t in (_JQ '(template_string) @t' 't')) {
        if ($t.code -cmatch '(?<![\w])(query|mutation|subscription|fragment)\s' -and $t.code -match '\{') { [void]$gql.Add(('{0}   <=  {1}' -f (_T $t.code), (_Src $t.file))) }
    }
...
function Get-ActiveTargets {
    # THE single active-scope chokepoint. Turns raw discovered candidates (absolute URLs or site-relative /paths)
    # into the canonical https URLs authorised for ACTIVE contact - the target host + its www/non-www counterpart
    # ONLY (Get-ActiveHosts). Off-host URLs are dropped; protocol-relative //host refs are dropped (ambiguous host);
    # site-relative paths are expanded onto each allowed host. EVERY active feature MUST build its request list
    # through this so a new feature cannot widen active scope by accident. Returns a HashSet[string].
    ...
    foreach ($e in @($Candidates)) {
        ...
        if ($e -match '^https?://') { $eh = ''; try { $eh = ([uri]$e).Host.ToLower() } catch {}; if ($eh -and $allowed.Contains($eh)) { [void]$out.Add($e) } }
        elseif ($e.StartsWith('/') -and -not $e.StartsWith('//')) { foreach ($ah in $allowed) { [void]$out.Add("https://$ah$e") } }
    }
    return $out
}
function Get-Apex {
...
function Phase6-Js {
    ...
    if ($jsl) {
        ...
        # map each archived body -> its real URL (unwrap waymore's archive/urlscan index) so deep-pass findings cite the source
        $jUrlMap = @{}
